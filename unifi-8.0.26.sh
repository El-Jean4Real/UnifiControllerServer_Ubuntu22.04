...
add_mongodb_repo() {
  ...
  if [[ "${mongodb_org_v::2}" == '70' ]] || [[ "${add_mongodb_70_repo}" == 'true' ]]; then
    ...
    if [[ "${try_different_mongodb_repo}" == 'true' ]] || [[ "${architecture}" != "amd64" ]]; then
      ...
      if [[ "${os_codename}" =~ (stretch|continuum|buster|bullseye|bookworm|trixie|forky) ]]; then
        mongodb_codename="debian bullseye"
        mongodb_repo_type="main"
    ...
    if echo "deb [ ${arch}${signed_by_value}${trusted_mongodb_repo} ] ${repo_http_https}://repo.mongodb.org/apt/${mongodb_codename}/mongodb-org/${mongodb_version_major_minor} ${mongodb_repo_type}" &> "/etc/apt/sources.list.d/mongodb-org-${mongodb_version_major_minor}.list"; then
      ...
      if [[ "${mongodb_key_update}" != 'true' ]]; then
        ...
        if [[ "${mongodb_org_upgrade_to_version::2}" == "${mongo_version_max}" ]]; then
          ...
          echo -e "${RED}#${RESET} Couldn't find MongoDB version \"${mongo_version_max_with_dot}\" in apt cache...\\n"
          sleep 3
        fi
      fi
    else
...
get_distro() {
  ...
  elif [[ "${os_codename}" =~ ^(bookworm|lory)$ ]]; then repo_codename=bookworm; os_codename=bookworm
  else
...
if ! dpkg -l jq 2> /dev/null | awk '{print $1}' | grep -iq "^ii\\|^hi"; then
  ...
  if ! DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' install jq &>> "${eus_dir}/logs/required.log"; then
    ...
    elif [[ "${repo_codename}" =~ (focal|groovy|hirsute|impish) ]]; then
...
if [[ "${first_digit_unifi}" -gt '8' ]] || [[ "${first_digit_unifi}" == '8' && "${second_digit_unifi}" -ge "1" ]]; then
  mongo_version_supported="7.0.999"
...
# arm64 override, always stick to 4.4.18.
if [[ "${architecture}" == "arm64" && "${mongo_version_max}" =~ (44|70) ]]; then
  mongo_version_max="44"
  mongo_version_max_with_dot="4.4"
fi
...
libssl_installation_check() {
  ...
  if [[ -z "${required_libssl_version}" ]]; then
    if [[ "${mongodb_org_upgrade_from_version::2}" -ge "36" && "${mongodb_package_requirement_check}" == 'true' ]]; then
      ...
      if grep -ioq "jammy" "/etc/apt/sources.list.d/mongodb-org-${mongodb_version_major_minor}.list"; then
...
mongodb_installation() {
  ...
  if DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' install "mongodb-org-server${install_mongodb_version_with_equality_sign}" "mongodb-org-shell${install_mongodb_version_with_equality_sign}" "mongodb-org-tools${install_mongodb_version_with_equality_sign}" &>> "${eus_dir}/logs/mongodb-org-install.log"; then
    ...
    if DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' install "mongodb-org-server${install_mongodb_version_with_equality_sign}" "mongodb-org-shell${install_mongodb_version_with_equality_sign}" "mongodb-org-tools${install_mongodb_version_with_equality_sign}" &>> "${eus_dir}/logs/mongodb-org-install.log"; then
      ...
      mongodb_package_version_libssl="${install_mongodb_version}"
      libssl_installation_check
      echo -e "${WHITE_R}#${RESET} Trying to install mongodb-org version ${mongo_version_supported::3} in the third run..."
      if DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' install "mongodb-org-server${install_mongodb_version_with_equality_sign}" "mongodb-org-shell${install_mongodb_version_with_equality_sign}" "mongodb-org-tools${install_mongodb_version_with_equality_sign}" &>> "${eus_dir}/logs/mongodb-org-install.log"; then
        echo -e "${GREEN}#${RESET} Successfully installed mongodb-org version ${mongo_version_supported::3} in the third run! \\n"
        mongodb_installed="true"
      else
        echo -e "${RED}#${RESET} Failed to install mongodb-org version ${mongo_version_supported::3} in the third run...\\n"
        abort
      fi
    fi
  fi
  ...
  if [[ "${broken_unifi_install}" == 'true' ]]; then
    ...
    if [[ -n "${previous_mongodb_version}" ]]; then
      ...
      if [[ "${previous_mongodb_version::2}" == '26' ]]; then
        ...
        mongo_version_supported="3.2.999"
        mongo_version_not_supported="4.0"
        ...
        mongo_version_supported="5.0.999"
        ...
        mongo_version_supported="6.0.999"
...
if [[ "${custom_unifi_deb_file_required}" == 'true' ]]; then
  ...
  echo -e "${WHITE_R}#${RESET} This setup is using temurin-${required_java_version_short}-${temurin_type}... Editing the UniFi Network Application dependencies..."
  if dpkg-deb -x "${unifi_temp}" "${eus_temp_dir}" &>> "${eus_dir}/logs/unifi-custom-deb-file.log"; then
    if dpkg-deb --control "${unifi_temp}" "${eus_temp_dir}/DEBIAN" &>> "${eus_dir}/logs/unifi-custom-deb-file.log"; then
      if [[ -e "${eus_temp_dir}/DEBIAN/control" ]]; then
        ...
        if sed -i "s/openjdk-${required_java_version_short}-jre-headless/temurin-${required_java_version_short}-${temurin_type}/g" "${eus_temp_dir}/DEBIAN/control" &>> "${eus_dir}/logs/unifi-custom-deb-file.log"; then
...
if [[ "${architecture}" == "arm64" && "${first_digit_unifi}" -ge "8" ]]; then
